...
from .script import default_evaluation_params


def calc_icdar_det_metrics(pred_bboxes_dict, gt_bboxes_dict, transcriptions_dict=None,
    ...
    TODO: quadrilateral이나 poligon 등의 형식으로도 계산할 수 있도록 변경해야 함.
    """
    ...
    def one_to_many_match(gtNum):
        ...
        if many_sum >= eval_hparams['AREA_RECALL_CONSTRAINT']:
    ...
    def many_to_one_match(detNum):
        ...
        if many_sum >= eval_hparams['AREA_PRECISION_CONSTRAINT']:
            return True,gtRects
        return False,[]
    ...
    #TODO: CRAFT에서는 dict타입으로 나오나봄... -> 어떻게 DBNet이랑 결합하지?
    ...
    for sample_name in gt_bboxes_dict:
        ...
        gt_bboxes = gt_bboxes_dict[sample_name]
        pointsList = gt_bboxes
        ...
        for n in range(len(pointsList)):
            ...
            dontCare = transcriptionsList is not None and transcriptionsList[n] == '###'
            if dontCare:
