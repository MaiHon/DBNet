import os, sys
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(os.path.abspath(os.path.dirname(os.path.dirname(__file__)))))))
...
def _resize_image_and_masks(image, self_min_size, self_max_size, target):
    ...
    if "masks" in target:
        target["masks"] = F.interpolate(target["masks"][:, None], scale_factor=scale_factor,
                             recompute_scale_factor=True)[:, 0].byte()
        target["shape"] = target["masks"].shape[-2:]
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def forward(self,
        ...
        # add to padding according to maximum image size in batch
...
if __name__ == "__main__":
    transfrom = GeneralizedRCNNTransform(min_size=[400, 500, 600], max_size=1400)

    for i in range(10):
        images = torch.randint(256, (2, 3, 1000, 600)).float()
        print(images.shape)
