...
from src.modules.utils import Boxes


def get_selfarea_and_interarea(boxes1, boxes2):
    """
...
class MultiFusePath(nn.Module):
    ...
    """
    def __init__(self, chans, **kwargs):
        super(MultiFusePath, self).__init__()

        self.chans = chans
        self.char_conv3x3 = nn.Conv2d(self.chans, self.chans, kernel_size=3, stride=1,
                                      padding=1, bias=False)
        self.char_conv1x1 = nn.Conv2d(self.chans, self.chans, kernel_size=1, stride=1,
                                      padding=0, bias=False)

        self.text_conv3x3 = nn.Conv2d(self.chans, self.chans, kernel_size=3, stride=1,
                                      padding=1, bias=False)
        self.text_conv1x1 = nn.Conv2d(self.chans, self.chans, kernel_size=1, stride=1,
                                      padding=0, bias=False)

        self.conv3x3 = nn.Conv2d(self.chans, self.chans, kernel_size=3, stride=1,
                                 padding=1, bias=False)
        self.conv1x1 = nn.Conv2d(self.chans, self.chans, kernel_size=1, stride=1,
                                 padding=0, bias=False)
        self.bn = nn.BatchNorm2d(self.chans)
